...
// content roughly fills the FOUNDING_SIZE hitbox; padding overflows transparently.
const SPRITE_OVERSCALE = 1.9;

export class FoundingSpriteInstance extends SpriteInstance {
  draw(ctx: CanvasRenderingContext2D, x: number, y: number, scale: number = 1) {
    ...
    const hitboxSize = GAME_CONFIG.FOUNDING_SIZE * scale;
...
export const foundingSprite = new FoundingSpriteInstance(foundingAnimations, "unlit");
