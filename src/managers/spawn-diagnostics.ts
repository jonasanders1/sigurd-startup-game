...
(window as any).spawnDiagnostics = {
  dump: () => {
    ...
    if (gameManager?.monsterSpawnManager && gameManager?.monsterRespawnManager) {
      const diagnostics = new SpawnDiagnostics(
        gameManager.monsterSpawnManager,
        gameManager.monsterRespawnManager
      );
      diagnostics.dumpFullDiagnostics();
    } else {
      console.error("Game managers not found. Is the game running?");
    }
  },
  ...
  stop: () => {
    ...
    if (instance) {
      ...
      console.error("No diagnostic instance running");
    }
  }
};